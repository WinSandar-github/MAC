...
class CPAFFController extends Controller
{
    ...
    public function storeRenewForm(Request $request){
        ...
        if($oldCpaff->offline_user==0){
            ...
            if($thisYear == $oldYear){
                $invoice->productDesc     = 'Application Fee, Renewal Fee, CPA(Full-Fledged) Registration';
                $invoice->amount          = $fees->form_fee.",".$fees->renew_fee;
            }else if($thisYear == $oldYear + 1 && date('M') === 'Jan'){
                $invoice->productDesc     = 'Application Fee, Renewal Fee, Delay Fee(within Jan), CPA(Full-Fledged) Registration' ;
                $invoice->amount          = $fees->form_fee.",".$fees->renew_fee . ',' . $fees->late_fee ;
            }
            else if($thisYear == $oldYear + 1 && date('m')>1 && date('m')<=4){
                $invoice->productDesc     = 'Application Fee, Renewal Fee, Delay Fee(from Feb to Apr), CPA(Full-Fledged) Registration' ;
