...
class PAPPController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $invoice->productDesc = 'App Fee , Reg Fee,PAPP Registration';
    ...
    public function PappRenewRegistration(Request $request){
        ...
        if($oldPapp->offline_user==0){
            ...
            if($diff>0){
                ...
                if($thisMonth === 'Jan'){
                    $invoice->productDesc     = 'App Fee, Renew Fee, Delay Fee,Reconnect Fee, PAPP Renewal Registration' ;
                    ...
                    $invoice->productDesc     = 'App Fee, Renew Fee,Reconnect Fee,  PAPP Renewal Registration';
                    $invoice->amount          = $fees->form_fee.",".$fees->renew_fee.",".$calculate_amount;
                }
            }
            else{
                if($thisYear == $oldYear){
                    $invoice->productDesc     = 'App Fee, Renew Fee, PAPP Renewal Registration';
                    $invoice->amount          = $fees->form_fee.",".$fees->renew_fee;
                }else if($thisYear == $oldYear + 1 &&  $thisMonth === 'Jan'){
                    $invoice->productDesc     = 'App Fee, Renew Fee, Delay Fee, PAPP Renewal Registration' ;
                    $invoice->amount          = $fees->form_fee.",".$fees->renew_fee . ',' . $fees->late_fee ;
                }
