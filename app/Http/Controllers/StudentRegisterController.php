...
class StudentRegisterController extends Controller
{
    ...
    public function privateSchoolReg(Request $request)
    {
        return $request->id;
    }

    public function macReg(Request $request)
    {
        if ($request->id != null) {
            $student = StudentRegister::with('student_info', 'course')
                ->where('student_info_id', '=', $request->id)
                ->first();

            return view('pages/da_registration_approval/da1_reg_individual', compact('student'));
        } else {
            return "no data";
        }
    }

    public function selfStudyReg(Request $request)
    {
        return $request->id;
    }
}
